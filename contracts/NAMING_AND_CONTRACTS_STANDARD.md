...
since: 2026-07-19 (G1)
delta: 2026-08-08 (R3 — OD-R3-SCOPE-2) — §2 توضيح دورة الحياة: دلالة Candidate/Approved + تثبيت مسار الاعتماد «توقيع SA أو بوابة FP معتمدة» بحسم OD-R3-SCOPE-2 — بلا ترقية أي عقد
...
**توضيح دورة الحياة (‏Δ 2026-08-08 · R3 — حسم `OD-R3-SCOPE-2`) — قواعد قابلة لإعادة الاستعمال:**
1. **`Candidate` يبقى تعييناً ضمن `Proposed`** — عقدٌ قيد المراجعة **أو** يحمل **تبعيات حاكمة مسمّاة** غير محسومة؛ ولا يُقرأ نقصاً في الأهلية للتحديث.
2. **مسار الاعتماد محسوم:** **`SA SIGNATURE OR APPROVED FP GATE`** — توقيع SA **أو** بوابة FP معتمدة، **لا اجتماعهما شرطاً**. (حسمُ تعارض `OD-R3-CONTRACTS-1` §(ب) بقرار المالك `OD-R3-SCOPE-2`؛ وقالب عقد الشاشة مُواءَم على النص نفسه.)
3. **دلالة `Approved` توثيقية:** التبعيات التوثيقية اللازمة للاعتماد **محسومة** والمواصفة **جاهزة للتنفيذ**. **ولا يُثبت `Approved` وجودَ تنفيذٍ ولا نجاحَ اختبارٍ تشغيلي.**
4. **أدلة المطابقة التشغيلية** (تنفيذاً واختباراً) **ملكُ بوابة التنفيذ/FP المعنية** — لا تُحمَّل على قرار الاعتماد التوثيقي.
5. **ولم يُرقَّ أيُّ عقد قائم بهذا التوضيح** — العقود الخمسة الحالية تبقى **`Candidate`**، وترقيتها محكومة إضافةً بقيد الترحيل في `OD-R3-SCOPE-2` §(و) (‏ترحيل `UI-4`/`UI-5` وتحديث الإحالات قبل أي ترقية).
