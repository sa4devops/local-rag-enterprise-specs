...
> مدخلات التصميم المعتمدة تاريخياً (v0.3): phases/PHASE_MASTER_PLAN.md وphases/designs/ — تُقرأ عند التفعيل ولا تُنفَّذ مباشرة.
- **مسار مؤقت معلن (Δ 2026-07-11 — ADR-0030):** «Open WebUI interim client» — واجهة محادثة مؤقتة معزولة في `platform/integrations/openwebui/` خلف عقود المنصة حصراً (مبدأ العميل القابل للاستبدال — ADR-0029). **لا يلغي أي مرحلة ولا يقدّم أي خدمة Backend قبل أوانها**: تفعيله خلف بواباته (ترخيص الصورة + السياسات)، وخروجه بشرط توفر الحد الأدنى من واجهة React البديلة وفق `DECOMMISSION_PLAN.md`؛ واجهة React تبقى خط الأساس المعتمد (D1/ADR-0017) دون إعادة فتح.
...
**Phase Exit Review (قاعدة مركزية):** لا تُعلن أي مرحلة من هذه الخارطة مغلقة، ولا تُصدر عبارة الانتقال إلى ما بعدها، قبل اجتياز Phase Exit Review في `../methodology/PHASE_EXECUTION_STANDARD.md` §3؛ والمراحل التي لا ملف تصميم مستقل لها بعد **مغطاة مركزياً من ذلك الموضع** عند إنشائها أو تفعيلها.
