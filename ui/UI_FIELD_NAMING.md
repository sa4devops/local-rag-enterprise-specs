...
> **Version:** 1.2 — (Δ 2026-08-02 R3: +القسم «الاسم الدلالي المحكوم مقابل النص الظاهر» نقلاً حاكماً لمبدأ `OD-R3-UI-3` — `screen_id`/`screen_name` يبقيان **الاسم الدلالي المحكوم على مستوى الشاشة**، والنص الظاهر عبر **i18n** حصراً، والاسم المتاح لكل سطح **قابل للتتبع إلى `screen_id`**. **الأنواع الثمانية في §1 بلا مساس** · **لا معرّف أُعيدت تسميته** · حقول السطح نفسها في `contracts/screens/SCREEN_CONTRACT_TEMPLATE.md`) (1.1: Δ v0.8: +سلسلة «الإرجاع الموجَّه» في §3 — WORKFLOW_ITEM_RETURNED · workflow.return_route — وفق FR-3.11/3.12) · **Date:** 2026-07-10 (الأصل 2026-07-06) · **الموضع الهدف:** `ui/UI_FIELD_NAMING.md`
...
---
## Δ v1.2 — الاسم الدلالي المحكوم مقابل النص الظاهر (‏R3 — نقل حاكم لمبدأ `OD-R3-UI-3`)

> **المصدر التاريخي غير الحاكم:** `GitHub Issue #11` → التعليق `OD-R3-UI-3` (‏`5156531912`). **الوعاء الحاكم للمبدأ هو هذا القسم**، **ووعاء الحقول** `contracts/screens/SCREEN_CONTRACT_TEMPLATE.md`؛ والتعليق **دليل دائم لا وعاء**.
> **السند:** `decisions/open-decisions.md` → **`OD-R3-SCOPE-1`** · `handoff/handoff.md` → **H-0028**.
> **طبيعة الدلتا:** **مبدأ تسميةٍ يفصل طبقتين قائمتين**، لا نوع تسمية تاسع. **الأنواع الثمانية في §1 بلا مساس، ولا معرّف يُعاد تسميته.**

### 1) الطبقتان

| الطبقة | المستوى | المعرّفات | الوظيفة |
|---|---|---|---|
| **الاسم الدلالي المحكوم** | **الشاشة** | **`screen_id`** · **`screen_name`** | مرجع **المعنى التجاري والقانوني** و**ربط الـAPI** و**الصلاحية** و**دلالة التدقيق** |
| **النص الظاهر** | **السطح** (‏عنصر من `surfaces[]`) | **`presentation_label_key`** — مفتاح i18n | ما يقرأه المستخدم على ذلك السطح |

### 2) قواعد الاسم والإعلان — ملزمة

1. **يجوز اختلاف النص الظاهر أسلوبياً**، **ولا يجوز** أن يغيّر **المعنى** ولا **الأثر القانوني** ولا **ربط الـAPI** ولا **الصلاحية** ولا **دلالة التدقيق**.
2. **يجب أن يبقى الاسم المتاح لكل سطح قابلاً للتتبع إلى `screen_id`.**
3. **لا يجوز كتابة عنوان أو اسم Surface مباشرة في الكود خارج i18n** — امتدادٌ نافذ لقاعدة §4 («لا نص مضمّن في المكوّنات») على مستوى الأسطح.
4. **مصدر الاسم المتاح** يُعلَن بالحقل `accessible_name_source` ∈ { `VISIBLE_TEXT` · `ARIA_LABEL` · `ARIA_LABELLEDBY` · `LIVE_REGION` }. وعند **`LIVE_REGION`** يأتي الاسم **من المحتوى الظاهر**، **دون `aria-label` منفصلة ودون حصر تركيز**.
5. **قاعدة الاشتقاق النصي** (‏`SCREEN_CONTRACT_TEMPLATE.md` §الغرض) تبقى سارية على **الاسم الدلالي المحكوم**، **لا على النص الظاهر**.

### 3) أثر النقل — ما يصير نافذاً عند الدمج

- **تعريفات الحقول** (`presentation_label_key` · `accessible_name_source` · `focus_return_target`) **موضعها** `contracts/screens/SCREEN_CONTRACT_TEMPLATE.md` — **لا تُكرَّر هنا**.
- **يتيح لاحقاً** تحويل `R3-A11Y-T-001` من **تطابق حرفي مع الجرد** إلى **تتبّع قابل للإثبات إلى `screen_id`**. **والتحويل نفسه لم يقع في هذه الجولة** — `traceability/r3-accessibility/**` **لم تُمسّ**.

> **حدود هذا القسم:** لا تقنين للدلتا السبعة · لا معيار `PASS` · لا معالجة ولا بناء اختبارات · لا رفع للتجميد ولا فكّ حجب `FP-0001` · `ui/UI_DESIGN_SYSTEM.md` **لم يُمسّ**.
