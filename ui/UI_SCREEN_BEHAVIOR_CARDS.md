...
> **قواعد قراءة:** كل بطاقة تحوي 9 حقول ثابتة. **قاعدة التغطية (‏Δ 2026-08-08 · `OD-R3-SCOPE-2`): التغطية المطلوبة كاملة — بطاقة سلوك لكل شاشة من شاشات الجرد الحالية (51) بنفس القالب**؛ القائم الآن **25 بطاقة** والمتبقي **26** يُستكمل ضمن مسار `R3` قبل إغلاقها التوثيقي — **ولم تُؤلَّف بطاقة في دفعة هذا التحديث**.
...
- **modal:** «إعادة إسناد»/«إرجاع إلى…» (Δ v0.8 — FR-3.12) ⇒ modal **بقائمة مرشّحين مرشّحة خادمياً** (صلاحية المرحلة + التصنيف/need-to-know + SoD؛ الافتراضي: معدّ المسودة) + **سبب إلزامي** — يظهر للمكلَّف الحالي حامل `workflow.return_route` فقط؛ الحدث `WORKFLOW_ITEM_RETURNED` + إشعار ثلاثي.
...
- **افتراضياً يظهر:** ملخص البند + الأثر المعلن + مقدِّم البند + السياسة المعمول بها + شريط قرارات (موافقة/رفض/**إرجاع إلى…**) + thread أسفل. (Δ v0.8 — OD-WF-1: الرفض **نهائي** والإرجاع **إعادة للمعالجة** — دلالتان متمايزتان في الشريط.)
- **مطويّ:** تفاصيل تقنية (payload كامل للأفعال) · مرفقات · تاريخ التعديلات.
- **حسب الصلاحية:** الأزرار معطَّلة إن كان الناظر هو المقدِّم (SoD) بسبب صريح · حقول payload حساسة مقنَّعة · اختيار وجهة إرجاع غير الافتراضي يتطلب `workflow.return_route` (المكلَّف الحالي — OD-WF-2).
- **drawer:** «سياسة الاعتماد المفصَّلة» ⇒ drawer end (للمخوّل) · «تدقيق البند» ⇒ drawer end للمخوّل.
- **modal:** «رفض» بلا تعليق ⇒ رفض النموذج + تنبيه · «موافقة على فعل critical» ⇒ modal typed · **«إرجاع إلى…» (Δ v0.8 — FR-3.11/3.12)** ⇒ modal: الوجهة الافتراضية **معدّ المسودة** + قائمة مرشّحين مرشّحة خادمياً (صلاحية المرحلة + التصنيف/need-to-know + SoD — من لا يجتازها لا يظهر) + **سبب إلزامي**؛ الحدث `WORKFLOW_ITEM_RETURNED` (from/to_state · from/to_user · reason) + إشعار ثلاثي + قيد في history.
...
## 24) `admin.org` — الهيكل التنظيمي (FR-1.Org-Ext) — (Δ v0.8)
- **افتراضياً يظهر:** شجرة تنظيمية **مرنة العمق** (department → section → unit عبر `parent_unit_id`) بأيقونة لكل `unit_type` · لوحة تفاصيل الوحدة المحددة (الاسم/النوع/العضوية) · شارة **توريث النطاق نزولاً** على الوحدات الوارثة.
- **مطويّ:** الوحدات الفرعية تحت مستوى العمق الثاني (توسيع بالنقر) · سجل تغييرات الوحدة (drawer للمخوّل).
- **حسب الصلاحية:** الشاشة كلها لحامل `admin.org.manage` · العضوية **على أي مستوى** تُدار من هنا حصراً · أزرار النقل/الدمج حسب قواعد البنية.
- **drawer:** تفاصيل عضوية وحدة (أعضاء + أدوار الوحدة) ⇒ drawer end · «تدقيق الوحدة» ⇒ drawer end للمخوّل.
- **modal:** «نقل وحدة» (سحب-إفلات أو زر) ⇒ modal تأكيد يعرض **أثر التوريث** على الفروع · «حذف/أرشفة وحدة» بها أعضاء ⇒ modal typed.
- **page:** هذه الشاشة page ضمن صدفة الـ Console · فتح ملف عضو ⇒ `admin.user_detail`.
- **متى إلى Renderer:** لا — سطح إداري خالص.
...
5. **الهوية مقفلة:** «Governed Enterprise AI Command Center» حكومي/رسمي — **ليس** تخطيطاً حضرياً، ليس صفحة تسويقية، ليس SaaS استهلاكياً، ليس chatbot استهلاكياً، ليس low-code builder للمستخدم النهائي. *(ضبط Δ v0.8 — A-UX-17: «ليس chatbot استهلاكياً» نفيٌ للهوية الاستهلاكية والمحادثة غير المحكومة؛ وضع المحادثة المحكوم OD-WS-4 مشروع، والأنماط المحادثية العامة مشاعة وفق UI_REFERENCE_USAGE_POLICY §3.1.)*
