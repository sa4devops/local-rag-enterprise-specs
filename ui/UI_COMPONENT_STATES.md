...
> **Version:** 1.2 — (Δ 2026-08-02 R3: +القسم «تفكيك حالة التعطيل — أنماط `S8` الإلزامية» نقلاً حاكماً لـ`OD-R3-UI-1` — **`S8` لا يُقبل بلا نمط**؛ و`S7` و`S9` و`S10` **بدلالاتها الحالية دون تغيير**؛ **لا معرّف `S` أُعيدت تسميته** · **جداول المكوّنات `S1`–`S15` والقسم `S16`–`S20` كما هي بلا مساس**) (1.1: Δ 2026-07-19 دفعة G1: +القسم S16–S20 «حالات البيانات/الاتصال على مستوى الشاشة» + مصفوفة انطباقها حسب الـ Archetype — جداول المكوّنات S1–S15 كما هي بلا مساس) · **Date:** 2026-07-19 (الأصل 2026-07-07) · **الموضع الهدف:** `ui/UI_COMPONENT_STATES.md`
...
> **المفتاح الموحّد للحالات (S1–S15 لكل مكوّن؛ ‏S16–S20 لمستوى الشاشة — القسم الختامي):** `default` (S1) · `collapsed` (S2) · `expanded` (S3) · `loading` (S4) · `empty` (S5) · `error` (S6) · `permission-denied` (S7) · `disabled` (S8) · `hidden` (S9) · `masked` (S10) · `confirmation` (S11) · `approval` (S12) · `success` (S13) · `failed` (S14) · `audit-visible` (S15).
...
---
## Δ v1.1 — حالات البيانات/الاتصال على مستوى الشاشة S16–S20 (دفعة G1)
> **طبيعة الدلتا:** الحالات الخمس التالية **تُعلَن على مستوى الشاشة/منطقة البيانات، لا على كل مكوّن** — فلا تُضاف أعمدة لجداول S1–S15 أعلاه. الإلزام عبر عقد كل شاشة (`contracts/screens/*`): تصريح بالانطباق أو استثناء معلَّل (`UI_SCREEN_GOVERNANCE_STANDARD §د`). قاعدة «غير المذكور لا يُصمَّم» سارية عليها.

| # | الحالة | ما يظهر | ما يُخفى/يُعطَّل | مصدر التبديل |
|---|---|---|---|---|
| **S16** | `offline` (انقطاع الاتصال) | شريط حالة اتصال ظاهر + آخر تحديث ناجح + زر إعادة محاولة؛ البيانات المعروضة تبقى مقروءة بوسم تقادم | **كل أفعال الكتابة تُعطَّل** (لا offline-write في v1)؛ لا مؤشرات «حي» | فشل نبض الاتصال/‏SSE أو فشل شبكي متكرر — كشف عميل، والعودة بالتحقق من الخادم |
| **S17** | `stale` (بيانات متقادمة) | شارة «آخر تحديث قبل …» على المنطقة + زر تحديث يدوي | مؤشرات «مباشر»؛ أي إبراز حداثة | تجاوز عتبة التقادم المعلنة في عقد الشاشة (افتراض Dashboard/Monitor: ظاهرة دائماً) |
| **S18** | `partial` (نجاح جزئي) | ما نجح يُعرض + لوحة «لم يكتمل: n بند» بأسبابها وزر إعادة للفاشل فقط | إيهام الاكتمال؛ ‏success عام | استجابة خادم دفعية/صفحية بنتائج مختلطة |
| **S19** | `saving` (كتابة قيد النقل) | مؤشر حفظ على مستوى النموذج/الصف المعني | **تعطيل مزدوج-الإرسال** (الزر المُطلِق ومكافئاته) حتى إقرار الخادم | بدء طلب كتابة → إقرار/فشل (يعود S13/S14 للمكوّن) |
| **S20** | `conflict` (نزاع إصدار) | حوار مقارنة النسختين (نسختي/نسخة الخادم بطابعيهما) + خيارا «استبدال بعد اطلاع» و«إلغاء وتحديث»؛ الدمج الحقلي إن أُتيح فبإعلان بالعقد | الحفظ الأعمى فوق نسخة أحدث | ‏version-check عند الحفظ (‏pessimistic-lite — ‏`UI_SCREEN_GOVERNANCE_STANDARD §ز`) |

### مصفوفة الانطباق الافتراضي حسب الـ Archetype (العقد يخصص)
| Archetype | S16 | S17 | S18 | S19 | S20 |
|---|---|---|---|---|---|
| List | ✔ | ✔ | ✔ (تحميل صفحات) | — | — |
| Detail | ✔ | ✔ | — | — | — |
| Form | ✔ | — | — | ✔ | ✔ |
| Builder | ✔ | — | ✔ | ✔ | ✔ |
| Canvas | ✔ | — | ✔ | ✔ | — |
| Dashboard | ✔ | **✔ دائمة الظهور** | ✔ | — | — |
| Monitor | **✔ مع مؤشر اتصال حي** | **✔ دائمة الظهور** | ✔ | — | — |
| Conversational | ✔ | ✔ (سياق الجلسة) | ✔ (أدوات جزئية) | ✔ (إرسال) | — |

**قاعدة إغلاق:** أي شاشة تدّعي «مكتملة» دون تصريح S16–S20 في عقدها = غير مكتملة عقدياً.

---
## Δ v1.2 — تفكيك حالة التعطيل: أنماط `S8` الإلزامية (‏R3 — نقل حاكم لـ`OD-R3-UI-1`)

> **المصدر التاريخي غير الحاكم:** `GitHub Issue #11` → التعليق `OD-R3-UI-1` (‏`5156528819`). **الوعاء الحاكم هو هذا القسم**؛ والتعليق **دليل دائم لا وعاء**.
> **السند:** `decisions/open-decisions.md` → **`OD-R3-SCOPE-1`** (سند استمرار Accessibility Sub-track — استثناء تنفيذي محدود) · `handoff/handoff.md` → **H-0028**.
> **طبيعة الدلتا:** **تفكيكُ حالةٍ قائمة إلى أنماط معلَنة، لا حالةٌ جديدة.** لا يُضاف معرّف `S` جديد، ولا يُعاد ترقيم شيء، ولا تُعدَّل جداول `S1`–`S15` ولا القسم `S16`–`S20` أعلاه.

### 1) الحالات التي تبقى بدلالاتها الحالية دون تغيير

| # | الحالة | ما يُثبَّت هنا |
|---|---|---|
| **S7** | `permission-denied` | يبقى بدلالته: **الإخفاء الناتج عن الصلاحية**؛ **لا يُرسل من الخادم ولا يبلغ المتصفح ولا شجرة الإتاحة** (‏`HIDDEN-SERVER`) |
| **S9** | `hidden` | **تبقى دلالته الحالية دون تغيير**، **ولا يُعلَن مرادفاً عاماً لـ`S7`**. وما ورد في جدول مكوّن بعينه يبقى **محصوراً بذلك المكوّن** ولا يُعمَّم |
| **S10** | `masked` | **يبقى بحاله** |

### 2) `S8 disabled` — نمط إلزامي واحد من أربعة

**قاعدة قاطعة: لا يُقبل `S8` بلا نمط.** كل إعلان لـ`S8` يحمل واحداً — وواحداً فقط — من الأنماط الأربعة:

| النمط | الدلالة | التركيز | إعلان السبب |
|---|---|---|---|
| **`S8:NATIVE`** | تعطيل أصيل | **قد لا يستقبل التركيز** — **بشرط** بلوغ السبب بوسيلة بديلة قابلة للوصول | إلزامي بوسيلة بديلة متاحة |
| **`S8:ARIA_FOCUSABLE`** | يبقى **قابلاً للتركيز** ولا ينفّذ الفعل | يستقبل التركيز | **يعلن سبب التعطيل في الاسم أو الوصف المتاح** — **لا عبر `hover-only tooltip`** |
| **`S8:READ_ONLY`** | القيمة **مقروءة ومتاحة للتصفح** | يستقبل التركيز للتصفح | **لا يعامَل كعنصر معطَّل** |
| **`S8:LOCKED`** | **متطلب سابق غير مستوفٍ** | **يبقى بالغاً بلوحة المفاتيح** | **يعلن المتطلب** |

**تطبيق ملزم:** حيث يفرض `ui/UI_ACTION_BUTTON_MODEL.md:24` (‏`disabled_when` — **سبب i18n**) سبباً مترجماً للتعطيل، **يُستخدم `S8:ARIA_FOCUSABLE`**. ويبقى الـ`tooltip` **خيار عرضٍ بصرياً لا بديلاً عن الإعلان المتاح**.

### 3) الحدود — تُقرأ مع القسم ولا تُتجاوز

1. **لا يُعاد تسمية أي معرّف `S`.**
2. **لا تُمسّ `S1`–`S7` ولا `S9`–`S20`** — جداولها أعلاه كما هي.
3. **اختيار النمط لكل عنصر يحدده العقد الحاكم لاحقاً** — لا يُحدَّد هنا ولا يُرتجل في التنفيذ.
4. **لا يعتمد سبب التعطيل على `hover` وحده.**

### 4) أثر النقل — ما يصير نافذاً عند الدمج

- **يمنع أي عقد شاشة من استخدام `S8` دون نمط** (‏الإنفاذ العقدي في `contracts/screens/SCREEN_CONTRACT_TEMPLATE.md`).
- **يتيح لاحقاً** تصحيح `R3-A11Y-02` و`R3-A11Y-T-010` لاختبار **النمط المعلَن** بدلاً من قاعدة منع تركيز مطلقة. **والتصحيح نفسه لم يقع في هذه الجولة** — `traceability/r3-accessibility/**` **لم تُمسّ**.

> **حدود هذا القسم:** **لا تقنين** للدلتا السبعة · **لا معيار `PASS`** · **لا معالجة** ولا بناء اختبارات · **لا رفع للتجميد** ولا فكّ حجب `FP-0001` · `ui/UI_DESIGN_SYSTEM.md` و`ui/UI_ACTION_BUTTON_MODEL.md` **لم يُمسّا**.
