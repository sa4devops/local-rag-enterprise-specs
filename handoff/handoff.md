...
**‏Δ 2026-08-06 (ب) — جولة تصحيح ثانية داخل PR #19: مواءمة بنية أسطر إيصال الدمج (‏`ADR-0033` بند 11) — لا مدخل handoff جديد**

- **ما أثبتته المراجعة المستقلة على الرأس `ddbde5e57b2b314976c9c7744aae31844cd8bc1a`:** نصُّ رسالة الـMerge Commit المتوقَّعة لـPR #19 **لا يحتوي السطر الفارغ الداخلي** الموجود في **سابقة سجلّ PR #18** بين سطر العنوان وسطر `Governance record`. **وقواعد المطابقة تطبّع الأسطر الفارغة الزائدة في البداية والنهاية فقط، ولا تطبّع السطر الفارغ الداخلي** ⇒ رسالةٌ بصيغة Git المعتادة كانت ستُقرأ اختلافاً في ترتيب الأسطر ⇒ **`MERGE RECEIPT MISMATCH` ⇒ لا دمج**.
- **ما صُحِّح:** أُضيف **سطر فارغ واحد فقط** بعد سطر العنوان وقبل `Governance record` داخل كتلة رسالة الدمج في `OD-R3-WF-ALLOC-1` §(ي)-2 — **فصارت بنيتُها مطابقةً لسابقة PR #18**. **دليل التغيير: `1 file changed, 1 insertion(+)`** — إدراجُ سطرٍ فارغٍ واحد، **بصفر حذف**.
- **ما لم يتغيّر:** **كلماتُ رسالة الدمج كما هي** · **ترتيبُ أسطرها غير الفارغة كما هو** · **قواعد المطابقة الأربع كما هي بلا تعديل** · **مصادقةُ المالك وأحكامُ الانحراف في §(ي)-1 بلا مساس** · **حقولُ السجل الحاكم السابق للدمج بلا مساس** · **الإصدار يبقى `3.9`**.
- **`REWORK_ON_FUNCTIONAL_CONTENT=NO`** — لم يُمسّ أيُّ محتوى وظيفي: **بطاقتا السلوك 13 و25 وعقد `admin.workflows` و`ui/**` و`contracts/**` و`methodology/**` و`decisions/adr/**` و`phases/**` و`traceability/**` كلُّها بلا تعديل في هذه الجولة.**
- **أثر التصحيح على أهلية الرأس:** **الرأس `ddbde5e5…` لم يعد مؤهلاً للدمج** بعد صدور commit التصحيح الثاني. **والرأس الجديد يحتاج مراجعةً مستقلة كاملة، ولا تأذن نتيجةُ أي مراجعة سابقة بدمجه** (‏`ADR-0033` بند 11: **لا يُدمج رأسٌ لم يحصل هو نفسه على `APPROVE`**).
- **نموذج الـcommit:** الـcommit الوظيفي الأصلي **يبقى ذرّياً واحداً**؛ ومعه **commitان تصحيحيان حاكمان** ⇒ **`EXPECTED_FINAL_PR_COMMIT_COUNT=3`**. **بلا amend ولا rebase ولا force-push ولا squash ولا حذف فرع ولا PR جديد.**
- **نطاق التصحيح المغلق:** ثلاثة ملفات — `decisions/open-decisions.md` · `handoff/handoff.md` · `INDEX.md`. **لا ملف جديد ولا حذف** · **ولا `Decision ID` ولا `Handoff ID` جديد**.
- **الحالة الحاكمة بلا مساس:** **لا دمج · لا وسم · لا إصدار · ولا إغلاق لـ`R3`** · **`FP-0001` `BLOCKED`** · **التجميد `ACTIVE` وشروط الرفع بلا تعديل ولا إعادة عدّ** · **`R4`/`R5` `NOT_ACTIVATED`** · `INDEX.md` **بلا صفّ جديد وبلا تغيير في الإجمالي** (‏158 + الفهرس · **EC-3 = 159/159**).

## Handoff H-0038
> **‏GOV — Execution Policy Persistence · ترسيخ سياسة تنفيذ تشغيلية لجلسات Claude Code.** يُقرأ بعد `H-0037` **ولا ينسخه ولا يعيد كتابته**: `H-0037` صحيحٌ في تاريخه وقد سجّل دفعة `R3`/`OD-R3-WF-ALLOC-1` (‏PR #19 — **دُمج**، وهو خط الأساس الحالي). **وهذه الجولة انحرافٌ مؤقتٌ حوكميّ (`TEMPORARY_DEVIATION — GOV`) خارج محتوى `R3`، لا تمسّ معماريتها ولا أي قرار منتج حاكم.**
> **تحقُّق ترقيم:** ‏`H-0038` **غير مستعمَل** في السجل عند تحرير هذا المدخل — تحقُّقاً من الملف لا افتراضاً؛ و`H-0016`..`H-0037` **لم تُمسّ**.

- date: 2026-08-07 (‏**تاريخ التنفيذ وتاريخ أمر المالك معاً**)
- phase: **‏GOV — Execution Policy Persistence** (خارج `R3` موضوعاً، وداخل نافذتها زمناً) — فرع جديد `ops/claude-execution-policy` من `BASE_SHA` مباشرة · **PR غير مدموج**
- task/goal: تنفيذ **أمر المالك** «‏**OWNER WRITE ORDER — PROJECT EXECUTION POLICY PERSISTENCE**»: إنشاء **`CLAUDE.md` في الجذر** وثيقةً **تشغيلية غير حاكمة** لجلسات Claude Code + **صفّها في `INDEX.md`** + **السجل الحاكم `OD-GOV-CLAUDE-1`** + **هذا المدخل**. **لا merge · لا tag · لا release · لا rebase · لا amend · لا force-push · لا حذف فرع · لا تعديل إعدادات مستودع ولا CI · لا رفع تجميد · لا فكّ حجب `FP-0001` · لا تفعيل `R4`/`R5` · لا إغلاق `R3` · ولم يُنشأ `contracts/screens/admin.workflow_detail.md`.**
- base SHA: **`origin/main` = `f774c621271b3d115b3efcc6710b20909f9971bc`** — **مطابقٌ حرفياً** للـSHA المثبَّت في أمر المالك، و`HEAD` عند إنشاء الفرع **مطابقٌ له قبل أي تعديل**.
- **خطية الفرع:** أُنشئ من `BASE_SHA` مباشرة · **commit واحد ذرّي** بأبٍ واحد · **صفر merge commit** · بلا rebase ولا amend ولا force-push.
- **نموذج الدفعة:** **`ONE_ATOMIC_COMMIT`** — الملف الجديد والفهرس وسجلّ القرار والـhandoff **في commit واحد**.

**التحقق من الأساس قبل أي كتابة — نتائجه:**

| الفحص | النتيجة |
|---|---|
| `origin/main` = ‏`BASE_SHA` | **مطابق** — `f774c621271b3d115b3efcc6710b20909f9971bc` |
| `git status --short` قبل التعديل | **فارغ** |
| `HEAD` بعد إنشاء الفرع وقبل أي تعديل | **مطابق لـ`BASE_SHA`** |
| الفرع `ops/claude-execution-policy` | **لم يكن موجوداً** على `origin` قبل هذه الجولة |
| **التجميد** | **`OD-FP-0001-FREEZE` = `ACTIVE`** — بلا مساس |
| **`FP-0001`** | **`BLOCKED`** — بلا مساس |
| **`R3`** | **`ACTIVE — SPECIFICATION ONLY`** — **لم تُغلق** وتبقى المسار النافذ |
| **`R4` / `R5`** | **`NOT_ACTIVATED`** — بلا مساس |
| **`contracts/screens/admin.workflow_detail.md`** | **غير موجود قبل الدفعة وبعدها** — **لم يُنشأ** |
| **`.claude/CLAUDE.md`** | **لم يُنشأ** — الموضع المعتمد هو الجذر وحده |

- completed: **`CLAUDE.md`** (**جديد** — 1.0: الإشعار الصدري بعدم الحوكمة + تسعة أقسام تشغيلية) · `INDEX.md` (**+صفٌّ جديد واحد** بعد `AUTHORITY.md` بترتيب ASCII + تحديث سطر الإجمالي **158 ⇒ 159 + الفهرس** + قيد Δ في الترويسة) · `decisions/open-decisions.md` (‏3.9 ⇒ **4.0**: +`OD-GOV-CLAUDE-1` بخمسة أقسام فرعية) · `handoff/handoff.md` (هذا المدخل).
- **أحكام المالك المسجَّلة:** `POINT_5=STOP_PLUS_DECISION_REQUEST` · `STANDING_CORRECTION_MANDATE=NO` · `GENERAL_PRECEDENT=NO` · **القبول لا يتعلق بنسبة إنجاز تقديرية مع بقاء `EC-3=100%` ملزماً** · **الموضع `/CLAUDE.md` لا `/.claude/CLAUDE.md`** · **`OWNER_AUTHORIZED_AGENT_MERGE`** · **وسوم الشدّة لا تحلّ محل تصنيفات §3.3 و`UNCLASSIFIED=0`**.
- **الوضع السلطوي لـ`CLAUDE.md`:** **تشغيليٌّ · غير حاكم · دون كل مصدر حاكم · بلا مرتبة سلطة · لا يضيف شيئاً إلى مصفوفة القراءة الحاكمة (`agent-execution-model` §9) · لا يُغني عن قراءة Tier-0 ولا القراءة الخاصة بالمهمة · ولا يعلو على أمر المالك النافذ ولا قائمة ملفاته ولا شروط توقفه.** **وأيُّ تعارض يُحسم لصالح `AUTHORITY.md` والمصدر الحاكم المعني.**
- not completed: **لم تتغيّر معمارية `R3`** ولا أيُّ قرار منتج حاكم · **لم تُمسّ `AUTHORITY.md` ولا `constitution.md` ولا `methodology/**` ولا `decisions/adr/**` ولا `contracts/**` ولا `ui/**` ولا `phases/**` ولا `catalogs/**` ولا `traceability/**`** · **لا ترقية عقد إلى `Approved` والمرتبة 5 تبقى خالية فعلياً و`OD-GOV-2` مفتوح** · **`contracts/screens/admin.workflow_detail.md` لم يُنشأ** · **لا معيار `R3-A11Y-01`..`07` أُعلن `PASS`** · **التجميد لم يُرفع** · **`FP-0001` = `BLOCKED`** · **`R4`/`R5` = `NOT_ACTIVATED`** · **`R3` لم تُغلق** · **لا اختبار بُني ولا نُفِّذ** (دفعةُ توثيقٍ صغيرة — **ولم تُنصَّب أدوات تحقق جديدة ولا تبعيات**) · **لا وسم ولا إصدار ولا دمج**.
- files changed: **أربعة** — `CLAUDE.md` (**جديد**) · `INDEX.md` · `decisions/open-decisions.md` · `handoff/handoff.md`. **ملفٌ جديد واحد أُنشئ ولم يُحذف شيء** ⇒ **صفٌّ جديد واحد في `INDEX.md`**، والإجمالي **159 ملفاً + الفهرس**، وملفات `.md` المتتبَّعة **160** · **EC-3 = 160/160 = 100%**.
- decisions: **قرار مالك واحد جديد** = **`OD-GOV-CLAUDE-1`** (‏`RECORDED — EFFECTIVE UPON MERGE`). **لا ADR · لا قرار معماري · لا إعادة ترقيم لأي `OD` قائم · لا تغيير لحالة أي `OD` آخر · ولم تُنشأ طبقة سلطة جديدة.**
- risks: (1) **قراءة `CLAUDE.md` سنداً حاكماً أو مرتبةً جديدة** — مُعالَج بالإشعار الصدري وبـ§9 فيه وبصفّ `INDEX.md` وبـ`OD-GOV-CLAUDE-1` §(أ) و§(ج)-1. (2) **قراءة §2 منه إذناً بأتمتة أو بدمج** — مُعالَج بنصٍّ صريح في §2: الأتمتة والمسارات المحمية محكومةٌ **حصراً** بـ`ADR-0033` وأمر المالك. (3) **قراءة §3 (عدم التوقف للمسائل منخفضة الأثر) تفويضَ تصحيحٍ ضمنياً** — مُعالَج بشروطه الخمسة وبـ§5 صراحةً: `STANDING_CORRECTION_MANDATE=NO`. (4) **قراءة وسوم الشدّة نظامَ تصنيفٍ موازياً** — مُعالَج بـ§7 وبـ`OD-GOV-CLAUDE-1` §(ب)-5: **ليست مترادفات**، وكلُّ ملاحظة تنال تصنيفاً حاكماً واحداً بـ`UNCLASSIFIED=0`. (5) **قراءة سطر `!.claude/CLAUDE.md` في `.gitignore` التزاماً بموضعٍ بديل** — مُعالَج بـ§(أ): **إذنٌ سابقٌ لا التزام**، **و`.gitignore` لم يُمسّ** لأنه خارج قائمة الملفات المأذونة.
- tests: **لا بوابات برمجية في هذا المستودع** (لا CI ولا سكربتات تحقق متتبَّعة) — والتحقق تمّ بأوامر `git` وفحوص قرائية: تطابق خط الأساس · نظافة الشجرة · حصر التغيير في الأربعة · **byte-identity** لكل المسارات المحمية مقابل `BASE_SHA` · تغطية `INDEX` (صفٌّ واحد لكل `.md` عدا الفهرس · بلا تكرار ولا يتيم · ستّ خلايا لكل صفّ) · `EC-3 = 160/160` · `git diff --check` · وغياب أي ادّعاء حوكمي في `CLAUDE.md`. **التفصيل في تقرير الدفعة.**
- **المراجعة:** جلسةُ التنفيذ أجرت **`Structured self-review`** فقط — **وهي ليست مراجعة مستقلة** ولا تستوفي `ADR-0033` بند 10-(ج) ولا `OD-GOV-MERGE-1` §(د)-3.
- next step: **مراجعة مستقلة** على الرأس النهائي (جلسة لم تكتب ولم تعدّل هذه الدفعة) ⇒ **`APPROVE`** ⇒ **Merge Agent مستقل** (جلسة ثالثة ليست المؤلف ولا المراجع) **بأمر مالك صريح منفصل يثبّت `PR number` و`expected head SHA`** (‏`ADR-0033` بند 10-(أ) وبند 11). **ولا يجوز لوكيل التنفيذ الذي كتب هذه الدفعة مراجعتُها ولا دمجُها.**
- **حالة الانحراف:** **`DEVIATION_STATUS=OPEN_PENDING_REVIEW_AND_MERGE`** — **وإنشاءُ الـPR وحده لا يُنهيه**. **`RETURN_NOW=NO`** · **هذه الجولة لا تعود إلى `R3` بنفسها** · **`RETURN_PATH=R3/contracts/screens/admin.workflow_detail.md`** — **وتأليفُ ذلك العقد يبقى محجوباً حتى أمرِ مالكٍ مستقل** (`OD-R3-WF-ALLOC-1` §(ط)-1).
- do not touch: constitution.md · AUTHORITY.md · methodology/** · decisions/adr/** · contracts/** · ui/** · phases/** · catalogs/** · traceability/** · references/** · knowledge/** · architecture/** · superseded/** · `.gitignore` · `.claude/**` · **`contracts/screens/admin.workflow_detail.md` (غير موجود — لا يُنشأ)** · **أي وسم أو إصدار** · **أي سجل تفعيل `R4` أو `R5`** · خط الأساس المجمَّد `188ad37` · الوسوم v1.0..v1.4 · **H-0016..H-0037**.

## Handoff H-0039
> **‏R3 — Contracts Coverage Expansion · `admin.workflow_detail` Candidate Screen Contract Authoring.** يُقرأ بعد `H-0038` **ولا ينسخه ولا يعيد كتابته**: `H-0038` صحيحٌ في تاريخه وقد سجّل انحراف `GOV` التشغيلي (‏PR #20 — **دُمج**، وهو خط الأساس الحالي)، **وسجّل مسار العودة `RETURN_PATH=R3/contracts/screens/admin.workflow_detail.md`**؛ **وهذه الجولة تنفّذ ذلك المسار بأمر مالك مستقل.**
> **تحقُّق ترقيم:** ‏`H-0039` **غير مستعمَل** في السجل عند تحرير هذا المدخل — **تحقُّقاً من الملف لا افتراضاً**؛ و`H-0016`..`H-0038` **لم تُمسّ**. وكذلك **`OD-R3-CONTRACTS-3` غير مستعمَل** قبل هذه الدفعة — تحقُّقاً من المستودع.

- date: 2026-08-07 (‏**تاريخ التنفيذ وتاريخ أمر المالك معاً**)
- phase: **R3 — Contracts Coverage Expansion** — فرع جديد `docs/r3-admin-workflow-detail-contract` من `BASE_SHA` مباشرة · **PR #21 غير مدموج**
- task/goal: **`admin.workflow_detail` Candidate Screen Contract Authoring** — تنفيذ **أمر المالك** «‏OWNER EXECUTION ORDER — R3 · `admin.workflow_detail` Candidate Screen Contract Authoring» (2026-08-07): **إنشاء `contracts/screens/admin.workflow_detail.md` بحالة `Candidate`** وفق `SCREEN_CONTRACT_TEMPLATE.md` **v1.1** بأقسامه الثمانية، مشتقّاً من **بطاقة السلوك 25** و**`OD-R3-WF-ALLOC-1`** · **ارتداد ضيّق** على العقد الشقيق · **تحديث خليّتين** في الجرد · **السجل الحاكم `OD-R3-CONTRACTS-3`** · **هذا المدخل**. **لا merge · لا tag · لا release · لا rebase · لا amend · لا force-push · لا squash · لا حذف فرع · لا PR ثانٍ · لا تعديل إعدادات مستودع ولا CI · لا رفع تجميد · لا فكّ حجب `FP-0001` · لا تفعيل `R4`/`R5` · لا إغلاق `R3`.**
- base SHA: **`origin/main` = `b49dd714e0b4da793e1f6d14d15d24f8aa87d2c6`** — **مطابقٌ حرفياً** للـSHA المثبَّت في أمر المالك، و`HEAD` عند إنشاء الفرع **مطابقٌ له قبل أي كتابة**.
- **actual PR number:** **`#21`** — `https://github.com/sa4devops/local-rag-enterprise-specs/pull/21` · base `main` · head `docs/r3-admin-workflow-detail-contract` · **مُلتقَطٌ من GitHub نفسه لا من تقدير**.
- **خطية الفرع:** أُنشئ من `BASE_SHA` مباشرة · كل commit **بأبٍ واحد** · **صفر merge commit** · بلا rebase ولا amend ولا force-push.
- **نموذج الدفعة:** **`PLANNED_FINAL_PR_COMMIT_COUNT=2`** — **مخطَّطٌ بأمر المالك لا تصحيحاً**، لأن **رقم الـPR لا يُعرف قبل إنشائه**: ‏(1) `R3: author admin.workflow_detail candidate contract` · (2) `R3: complete pre-merge record for admin.workflow_detail contract`.

**التحقق من الأساس قبل أي كتابة — نتائجه:**

| الفحص | النتيجة |
|---|---|
| `origin/main` = ‏`EXPECTED_BASE_SHA` | **مطابق** — `b49dd714e0b4da793e1f6d14d15d24f8aa87d2c6` |
| `git status --short` قبل التعديل | **فارغ** |
| `HEAD` بعد إنشاء الفرع وقبل أي كتابة | **مطابق لـ`BASE_SHA`** |
| **PR #19 · PR #20** | **`MERGED`** — و`Merge Commit` لـPR #20 هو `BASE_SHA` نفسه |
| `contracts/screens/admin.workflow_detail.md` قبل الدفعة | **غير موجود** — **أُنشئ في هذه الدفعة بأمر مالك مستقل** |
| **بطاقة السلوك 25** | **موجودة** — `ui/UI_SCREEN_BEHAVIOR_CARDS.md` §25 |
| **`OD-R3-WF-ALLOC-1`** | **موجود · `RESOLVED BY OWNER DECISION`** |
| **`OD-R3-CONTRACTS-3`** | **غير مستعمَل** قبل هذه الدفعة — **صفر تصادم** |
| **`H-0039`** | **غير مستعمَل** قبل هذا المدخل — **صفر تصادم** (أعلى معرّف قائم `H-0038`) |
| الفرع `docs/r3-admin-workflow-detail-contract` | **لم يكن موجوداً** على `origin` قبل هذه الجولة |
| **التجميد** | **`OD-FP-0001-FREEZE` = `ACTIVE`** — بلا مساس |
| **`FP-0001`** | **`BLOCKED`** — بلا مساس |
| **`R3`** | **`ACTIVE — SPECIFICATION ONLY`** — **لم تُغلق** |
| **`R4` / `R5`** | **`NOT_ACTIVATED`** — بلا مساس |
| **`contracts/fields/`** | **غير موجود** — **ولم يُنشأ** |

**التحقق القرائي من `aql` — مستودعٌ خارج الدفعة (المرتبة 11):** فُحص **`sa4devops/aql` عند `eab9f99569464cd65cbbac6141d783205ead5da8` تحديداً — لا عند `HEAD` متغيّر**، **قراءةً فقط**، **ولم يدخل في ملفات الدفعة**. والنتيجة: **لا مسار `/admin/workflows`** · **لا مسار `/admin/workflows/{id}`** · **لا تنفيذ مستقل لـ`admin.workflow_detail`** · **`src/app/workflow-canvas-builder` و`src/features/workflow/**` موجودان** (‏`ReactFlowCanvas.tsx` · `WorkflowPage.tsx`) بوصفهما **الـcanvas التجريبي غير المعتمد** · `activate` و`rollback` و`version diff` **بصفر ورود** · و`approve` **قيمةُ حالةٍ ونصُّ تدقيقٍ في بيانات وهمية لا فعلاً منفَّذاً**. **مطابقٌ للنتيجة المتوقَّعة في أمر المالك ⇒ لا `AQL_EVIDENCE_DRIFT`.** **ولم يُسجَّل انحرافٌ خاصٌّ جديد للشاشة**، **ولم يُقرأ غيابُ التنفيذ إعلانَ مطابقة**، و**`CC-WF-1` أُحيل إلى وعائه القائم `admin.workflows` §6 بلا تكرار وبلا حسم**.

- completed: **`contracts/screens/admin.workflow_detail.md`** (**جديد** — `0.1` · **`Candidate`**: §1/§2 مشتقّان نصاً من صفَّي الجرد A/B · §3 بطاقة 25 · **§3-أ `surfaces[]` سبعة** + جدول سند اشتقاق لكل سطح + «ما ليس سطحاً» أربعة بنود · §3-ب سلوك كل سطح + ست قواعد · §3-ج التعامد · §3-د الترتيب المنطقي المعلَن · §3-هـ نمطا `S8` · §4 حالات البطاقة · **§4-أ تصريح `S16`–`S20`** (‏`S17` استثناء معلَّل) · §5 حزمة `Builder` + عشرة بنود خاصة · **§6 نتيجة الفحص القرائي لـ`aql@eab9f99`** · §7 + **§7-أ ثماني تبعيات لاحقة** · §8) · `contracts/screens/admin.workflows.md` (‏`0.3 ⇒ **0.4**` — **ارتدادٌ ضيّق**: الترويسة/الدلتا + **§7** + **صفّ `DD-WF-DETAIL-SCREEN` في §7-أ** — **أربعة أسطر لا غير**) · `ui/UI_SCREEN_INVENTORY.md` (**خليّتان فقط**) · `decisions/open-decisions.md` (‏`4.0 ⇒ **4.1**`: +`OD-R3-CONTRACTS-3` بأربعة أقسام فرعية — **والأقسام السابقة لم تُمسّ**) · `handoff/handoff.md` (هذا المدخل) · `INDEX.md` (**+صفٌّ جديد واحد** + **تحديث صفوف قائمة**).
- **`surfaces[]` — الإعلان الناتج:** **سبعة قطعاً** — `page_workflow_detail` (`PAGE`) · `drawer_version_diff` (`DRAWER`) · `drawer_workflow_detail_audit_chain` (`DRAWER`) · `dialog_workflow_approval` (`DIALOG`) · `dialog_workflow_activation` (`DIALOG`) · `dialog_workflow_rollback` (`DIALOG`) · `dialog_workflow_detail_version_conflict` (`DIALOG`). **كلها من التعداد المغلق الخماسي** · **صفر سطح ثامن** · **صفر سطح `canvas`** · **`tab` لم يُعلَن سطحاً** · **`WIZARD_STEP`/`COMMAND_SURFACE`/`AGENT_CONVERSATION_SURFACE` بقيت مؤجَّلة ولم تُستعمل** · **`INLINE_PANEL` و`TRANSIENT` غير مستعمَلين**. و**كل سطح يحمل سنداً مشتقّاً منه** — **لا تأليف موازٍ**.
- **حقول الأسطح:** ‏`presentation_label_key` **معلَنٌ للسبعة** بمفاتيح i18n المثبَّتة في أمر المالك · `accessible_name_source` = **`VISIBLE_TEXT`** للـ`PAGE` و**`ARIA_LABELLEDBY`** للأسطح الستة القابلة للإغلاق (**ولا `LIVE_REGION`**) · `focus_return_target` = **`AUTO_TRIGGER`** للستة القابلة للإغلاق **حصراً**، **وغير معلَن حيث لا يوجبه القالب** (`PAGE`) · **صفر قيمة `null`** · **صفر استعمال لـ`<control_id>`**.
- **نمط التأكيد:** ‏`dialog_workflow_approval` و`dialog_workflow_activation` = **`typed`** (استهلاكاً لبطاقة 25) · **`dialog_workflow_rollback` = تأكيدٌ صريح بلا وسم نمط** — **لا `typed` ولا `dual` ولا `simple`**، **ولا قياس على استثناء `queue.approval_detail`**، **ولا فتحٌ تلقائي**.
- not completed / **ما لم يُحسم**: **العقد الجديد يبقى `Candidate`** — **لا ترقية ولا ادّعاء ترقية** · **المرتبة 5 تبقى خالية فعلياً و`OD-GOV-2` مفتوح** · **`OD-P3-1` و`OD-P3-2` (ومعه `CC-WF-1`) و`OD-P3-3` مفتوحة بلا مساس** · **لا سطح `canvas` ولا اعتماد لسلوك `aql`** · **لا مصفوفة اعتماد افتراضية** · **لا سياسة `instances` عند rollback** · **`DD-WF-DETAIL-VIEW-PERM` مسجَّل `RECORDED — NOT RESOLVED — NON-BLOCKING FOR CANDIDATE AUTHORING`** · **`DD-WF-FOCUS-RETURN` تحوّل ولم يُغلق** (‏+تسجيل توتّر `S12` **بلا وسم `Superseded` لأيٍّ من نصّيه**) · **`DD-WF-ACTION-CATALOG` و`DD-WF-STATUS-LIFECYCLE` (وجهته `R5`) و`DD-WF-CONTROL-ID` و`DD-WF-FIELDS` و`DD-WF-APPROVAL-PATH` و`DD-WF-A11Y-ENFORCEMENT` مفتوحة** · **`CC-WF-2`/`CC-WF-3` بوجهتهما `R5`** · **`OD-NM-1` لم يُحسم** · **لا معيار `R3-A11Y-01`..`07` أُعلن `PASS`** · **`contracts/fields/` لم يُنشأ** · **`contracts/enums/**` و`contracts/identity/**` والقالب لم تُمسّ** · **التجميد لم يُرفع** · **`FP-0001` = `BLOCKED`** · **`R4`/`R5` = `NOT_ACTIVATED`** · **`R3` لم تُغلق** · **لا اختبار بُني ولا نُفِّذ** · **لا وسم ولا إصدار ولا دمج**.
- files changed: **ستة** — `contracts/screens/admin.workflow_detail.md` (**جديد**) · `contracts/screens/admin.workflows.md` · `ui/UI_SCREEN_INVENTORY.md` · `decisions/open-decisions.md` · `handoff/handoff.md` · `INDEX.md`. **ملفٌ جديد واحد أُنشئ ولم يُحذف شيء** ⇒ **صفٌّ جديد واحد في `INDEX.md`**، والإجمالي **159 ⇒ 160 ملفاً + الفهرس**، وملفات `.md` المتتبَّعة **161** · **EC-3 = 161/161 = 100%**.
- decisions: **قرار مالك واحد جديد** = **`OD-R3-CONTRACTS-3`** (‏`RECORDED — EFFECTIVE UPON MERGE`) — **سجلُّ دفعةٍ صغير** يحمل الواقعة والقيود والتبعيات المفتوحة و**السجل الحاكم السابق للدمج لـPR #21**. **والمعرّف اختيارٌ تنظيميٌّ للمالك لهذه الدفعة — `ADR-0033` يوجب وجودَ السجل الحاكم لا هذا المعرّف بعينه.** **لا ADR · لا قرار معماري · لا إعادة ترقيم لأي `OD` قائم · لا تغيير لحالة أي `OD` آخر · ولم تُنشأ طبقة سلطة جديدة.**
- **رفعُ قيود دفعاتٍ سابقة — نطاقُه مغلق:** كانت `H-0036`/`H-0037`/`H-0038` تدرج `contracts/screens/admin.workflow_detail.md` و`ui/UI_SCREEN_INVENTORY.md` ضمن `do not touch` — **وتلك القيود كانت صحيحةً في دفعاتها**. **ورفعُ المالك في هذه الدفعة محصورٌ في مسارين اثنين لغرضين اثنين:** `contracts/screens/admin.workflow_detail.md` لأجل **`CREATE`** فقط · و`ui/UI_SCREEN_INVENTORY.md` لأجل **تعديل خليّتين بعينهما في صفّ `admin.workflow_detail`** فقط. **وهذا الرفع `IS NOT A GENERAL OVERRIDE` · `IS NOT A PRECEDENT` · `DOES NOT SUPERSEDE H-0036/H-0037/H-0038 HISTORICALLY`**، **وكلُّ قيود `do not touch` الأخرى بقيت نافذة بلا تخفيف**.
- risks: (1) **قراءة إنشاء العقد ترقيةً أو حسماً لـ`OD-GOV-2`** — مُعالَج بـ`status: Candidate` في الترويسة وبـ§(ج)-1 من السجل. (2) **قراءة ملاحظة الجرد «canvas مبسّط v1» إذناً بسطح canvas** — مُعالَج بقيد القراءة الصريح في §2 من العقد مستنداً إلى `OD-R3-WF-ALLOC-1` §(ط)-5، وبالبند 2 من «ما ليس سطحاً». (3) **قراءة `wf.approve` (SoD) صلاحيةَ عرضٍ أو استدعاءَ صلاحيةٍ ثالثة** — مُعالَج بـ§2 وبتسجيل `DD-WF-DETAIL-VIEW-PERM` **بلا اختراع ولا استعارة ولا جمع**. (4) **قراءة `AUTO_TRIGGER` إغفالاً لشرط الهدف البديل** — مُعالَج بالقاعدة 5 في §3-ب: **بيانُ سببٍ صريح** + **تسجيل توتّر `S12`** + **`DD-WF-FOCUS-RETURN` يتحوّل ولا يُغلق**. (5) **اشتقاق نمط تأكيدٍ لـrollback بالقياس** — مُعالَج بالقاعدة 4 في §3-ب وبإحالة الأفخاخ الثلاثة إلى `OD-R3-WF-ALLOC-1` §(و). (6) **قراءة غياب تنفيذ `aql` إعلانَ مطابقة** — مُعالَج بقاعدة القراءة الملزمة في §6: **غيابُ تنفيذٍ مستقل ليس إعلانَ مطابقة**. (7) **قراءة تعديل الجرد توسيعاً للنطاق** — مُعالَج بحصر التعديل في خليّتين مُثبَتٍ بـ`git diff --word-diff`. (8) **قراءة الـcommit الثاني «تصحيحاً» بعد مراجعة** — مُعالَج بنصّ أمر المالك و§(د) من السجل: **`PLANNED_FINAL_PR_COMMIT_COUNT=2` مخطَّطٌ سلفاً لأن رقم الـPR لا يُعرف قبل إنشائه**.
- tests: **لا بوابات برمجية في هذا المستودع** (لا CI ولا سكربتات تحقق متتبَّعة) — والتحقق تمّ بأوامر `git` و`gh` وفحوصٍ قرائية: تطابق خط الأساس **حرفياً** · نظافة الشجرة قبل الكتابة · `HEAD` = `BASE_SHA` قبل أول كتابة · **حصر التغيير في الستة** · **byte-identity** لكل المسارات المحمية مقابل `BASE_SHA` (`AUTHORITY.md` · `constitution.md` · `CLAUDE.md` · `methodology/**` · `phases/**` · `catalogs/**` · `traceability/**` · `references/**` · `knowledge/**` · `architecture/**` · `superseded/**` · `decisions/adr/**` · `contracts/enums/**` · `contracts/identity/**` · القالب · `NAMING_AND_CONTRACTS_STANDARD.md` · `run.list.md` · `admin.actions.md` · `admin.record_types.md` · بقية `ui/**`) · **`ui/UI_SCREEN_INVENTORY.md` = `1 insertion(+), 1 deletion(-)` وخليّتان فقط مُثبَتتان بـ`--word-diff`** · **`contracts/screens/admin.workflows.md` = أربعة أسطر فقط (5 · 9 · 180 · 192)** · `git diff --check` **نظيف** · **سبعة أسطح** و**كل `type:` من التعداد المغلق** · **صفر `null`** في حقول الأسطح · **`focus_return_target` = `AUTO_TRIGGER` للستة القابلة للإغلاق وغير معلَن للـ`PAGE`** · **صفر `<control_id>`** · **صفر سطح `canvas`** · **صفر وسم نمط لـrollback** · **`status: Candidate`** و**صفر ادّعاء `Approved`/`PASS`** · **صفر `action_id` أو صلاحية أو حدث تدقيق أو تعداد جديد** (الأحداث الثلاثة والأفعال الثلاثة والصلاحية الواحدة كما هي) · **صفر إعادة تسمية `wf.*` ⇐ `workflow.*`** · تغطية `INDEX` (**صفٌّ واحد لكل `.md` عدا الفهرس · صفر يتيم · صفر تكرار**) · **`EC-3 = 161/161 = 100%`** · **`OD-R3-CONTRACTS-3` و`H-0039` كلٌّ منهما مرّةً واحدة** · **رقم الـPR الفعلي `#21` مُلتقَطٌ من GitHub** · **نصّ رسالة الدمج المتوقَّع مثبَّت**. **التفصيل في تقرير الدفعة.**
- **المراجعة — تسجيل حالة:** جلسةُ التنفيذ أجرت **`Structured self-review`** فقط:
...
```
  ...
  **‏Execution session did not perform independent review.**
  **‏Execution session is not authorized to merge.**
  **ولا يجوز لوكيل التنفيذ الذي كتب هذه الدفعة مراجعتُها ولا دمجُها** (`agent-execution-model` §18.2-1 · `OD-GOV-MERGE-1` §(د)-3).
- next step: **مراجعة مستقلة لـPR #21** على الرأس النهائي — **جلسة/مراجعٌ لم يؤلّف هذه الدفعة ولم يدفع أي commit إلى فرعها** ⇒ **`APPROVE`** ⇒ ثم **Merge Agent مستقل** (جلسة ثالثة ليست المؤلف ولا المراجع) **بأمر مالك صريح منفصل يثبّت `PR number` و`expected final head SHA`** (‏`ADR-0033` بند 10-(أ) وبند 11) — **يطابق نصّ إيصال الدمج المثبَّت في `OD-R3-CONTRACTS-3` §(د) قبل التنفيذ**، ثم **تحقُّق قراءةً فقط بعد الدمج** بلا أي تحديث مستودعي لاحق (‏`ADR-0033` بند 12). **وأمرُ التنفيذ الحالي ليس تفويض دمج.**
- do not touch: constitution.md · AUTHORITY.md · CLAUDE.md · methodology/** · phases/** · catalogs/** · traceability/** · references/** · knowledge/** · architecture/** · superseded/** · decisions/adr/** · aql/** · `contracts/screens/SCREEN_CONTRACT_TEMPLATE.md` · `contracts/NAMING_AND_CONTRACTS_STANDARD.md` · `contracts/screens/run.list.md` · `contracts/screens/admin.actions.md` · `contracts/screens/admin.record_types.md` · `contracts/enums/**` · `contracts/identity/**` · **`contracts/fields/**` (غير موجود — لا يُنشأ)** · **بقية `ui/**` خارج الخليتين المأذونتين** (‏ومنها `UI_SCREEN_BEHAVIOR_CARDS.md` و`UI_ADMIN_CONSOLE_MODEL.md` و`UI_SCREEN_CARDS_BY_PHASE.md` و`UI_DESIGN_SYSTEM.md` و`UI_COMPONENT_STATES.md` و`UI_FIELD_NAMING.md` و`UI_INTERACTION_MODEL.md`) · **جدول B من الجرد وخليّةُ ملاحظات `admin.workflow_detail`** · **إعدادات المستودع وحماية الفروع وCI** · **أي وسم أو إصدار** · **أي سجل تفعيل `R4` أو `R5`** · خط الأساس المجمَّد `188ad37` · الوسوم v1.0..v1.4 · **H-0016..H-0038**.

## Handoff H-0040
> **‏R3 — Scope and Closure Definition Alignment (‏`OD-R3-SCOPE-2`).** يُقرأ بعد `H-0039` **ولا ينسخه ولا يعيد كتابته**: `H-0039` صحيحٌ في تاريخه — **PR #21 دُمج** وMerge Commit له هو خط الأساس الحالي، **مع انحراف إيصالٍ مسجَّل واقعةً** في `OD-GOV-MERGE-1` §(م).
> **تحقُّق ترقيم:** ‏`H-0040` **غير مستعمَل** في السجل عند تحرير هذا المدخل — **تحقُّقاً من الملف لا افتراضاً** (أعلى معرّف قائم `H-0039`)؛ وكذلك **`OD-R3-SCOPE-2` غير مستعمَل** قبل هذه الدفعة — تحقُّقاً من المستودع؛ و`H-0016`..`H-0039` **لم تُمسّ**.

- date: 2026-08-08 (‏**تاريخ التنفيذ وتاريخ أمر المالك معاً**)
- phase: **R3 — Enterprise UI Governance & Complete Screen Contract Coverage** (الاسم الحاكم الجديد بـ`OD-R3-SCOPE-2`) — فرع جديد `docs/r3-scope-closure-alignment` من `BASE_SHA` مباشرة · **وR3 تبقى `ACTIVE — SPECIFICATION ONLY` ولم تُغلق**
- task/goal: تنفيذ **أمر المالك** «‏OWNER EXECUTION ORDER v3 — R3 Scope and Closure Definition Alignment» (2026-08-08 · `OD-R3-SCOPE-2`): **مواءمة نطاق `R3` وتعريف إغلاقها توثيقياً** — حسم تعارض المرتبتين 2/4 · استبدال شروط الإغلاق القديمة (17)–(20) بعشرة شروط توثيقية · حسم مسار الاعتماد و`OD-GOV-2` · قيد ترحيل العقود الخمسة · الإحلال المستقبلي للنصوص التاريخية المكررة · تسجيل انحراف إيصال PR #21 واقعةً. **لا merge · لا tag · لا release · لا rebase · لا amend · لا force-push · لا squash · لا إغلاق `R3` · لا تفعيل `R4`/`R5` · لا رفع تجميد · لا فكّ حجب `FP-0001` · لا ترقية عقد · لا تقنين `UI-4`/`UI-5` · لا تأليف بطاقة ولا عقد · لا تنفيذ ولا اختبار ولا معالجة.**
- base SHA: **`origin/main` = `195407d3db09b68f41c91291224446902834f179`** — **مطابقٌ حرفياً** للـSHA المثبَّت في أمر المالك (‏Merge Commit لـPR #21)، و`HEAD` عند إنشاء الفرع **مطابقٌ له قبل أي كتابة**. **والرأس النهائي لا يُثبَّت داخل ملفٍّ في الدفعة** (‏`ADR-0033` بند 11) — يُؤخذ من مخرج المراجعة المستقلة ومن الأب الثاني للـMerge Commit.
- **actual PR number:** **`#22`** — `https://github.com/sa4devops/local-rag-enterprise-specs/pull/22` · base `main` · head `docs/r3-scope-closure-alignment` · **مُلتقَطٌ من GitHub نفسه لا من تقدير** — ثُبِّت في الـcommit الثاني المخطَّط (انظر «نموذج الدفعة» أدناه) مع **السجل الحاكم السابق للدمج** في `OD-R3-SCOPE-2` §(ل).
- **نموذج الدفعة:** **`PLANNED_FINAL_PR_COMMIT_COUNT=2`** — **مخطَّطٌ لا تصحيحاً**: ‏(1) دفعة المواءمة (تسعة ملفات) · (2) **استكمال السجل الحاكم السابق للدمج** بعد معرفة رقم الـPR الفعلي (‏`ADR-0033` بند 12).
- **خطية الفرع:** أُنشئ من `BASE_SHA` مباشرة · كل commit **بأبٍ واحد** · **صفر merge commit** · بلا rebase ولا amend ولا force-push.
...
| الفحص | النتيجة |
|---|---|
| `origin/main` = ‏`EXPECTED_BASE_SHA` | **مطابق** — `195407d3db09b68f41c91291224446902834f179` |
| `git status --porcelain` قبل التعديل | **فارغ** |
| لا PR مفتوح متداخل · الفرع `docs/r3-scope-closure-alignment` غير موجود | **متحقَّق** (‏`gh pr list` فارغ · `ls-remote` فارغ) |
| `OD-R3-SCOPE-2` · `H-0040` | **غير مستعمَلين** — صفر تصادم |
| `OD-R3-SCOPE-1` | **موجود**: الاستثناء **`ACTIVE`** · «‏`AUTHORITY CONFLICT — SEPARATE OWNER ACTION`» |
| `R3-ACT-01` | يحوي (1)–(16) والشروط القديمة (17)–(20) — `:388`..`:392` |
| الجرد A/B | **51 `screen_id` فريدة في A** · **والقيم الـ51 نفسها في B** (فحص آلي) |
| بطاقات السلوك | **25** |
| عقود `Candidate` | **خمسة حصراً** (عدا القالب): `run.list` · `admin.actions` · `admin.record_types` · `admin.workflows` · `admin.workflow_detail` — **ولا عقد `Approved`** |
| ملفات `.md` المتتبَّعة | **161 شاملةً `INDEX.md`** · **160 عداه** · **وINDEX يحوي 160 صفَّ بياناتٍ عدا صفّ الترويسة، كلُّ صفٍّ يحيل إلى ملف `.md` واحد** — وسطر الإجمالي الختامي كان **متقادماً** (159/160) وصُحِّح في هذه الدفعة |
| التتبع `R3-ACCESSIBILITY-SPECIFICATION.md` | **§7-ج يكرّر** الشروط القديمة (17)–(20) · **§8(هـ) يصف `OD-GOV-2` مفتوحاً** — كلاهما أُسند له الإحلال المستقبلي |
| العقود الخمسة | تحمل إحالات مؤرَّخة لتعارض مسار الاعتماد و/أو `OD-GOV-2` |
| رسالة Merge Commit لـPR #21 | **تخالف** الكتلة المتوقَّعة المحكومة في `OD-R3-CONTRACTS-3` §(د) — رسالة GitHub الافتراضية (سُجِّلت واقعةً في §(م)) |
| **التجميد** · **`FP-0001`** · **`R4`/`R5`** | **`ACTIVE`** · **`BLOCKED`** · **`NOT_ACTIVATED`** — بلا مساس |

- completed: **تسعة ملفات حصراً — وفق القائمة البيضاء حرفياً:** ‏(1) `methodology/RECONCILIATION_ROADMAP.md` (‏`1.3 ⇒ 1.4` — **صفّ `R3` وحده**: الاسم الحاكم الجديد + التغطية التوثيقية الكاملة للـ51 + أسس `UI-4`/`UI-5` + بطاقة وعقد لكل شاشة + `UNOWNED_TBD = 0` + **الإغلاق توثيقي بلا معالجة تشغيلية ولا أدلة تنفيذ** — **وصفوف `R1`..`R2` و`R4`..`R11` لم تُمسّ ولم يُفعَّل شيء**) · (2) `decisions/open-decisions.md` (‏`4.1 ⇒ 4.2`: +**`OD-R3-SCOPE-2`** بأقسامه (أ)..(ل) — ومنها **§(ل) السجل الحاكم السابق للدمج — PR #22** المستكمَل في الـcommit الثاني المخطَّط · +**§(م)** في `OD-GOV-MERGE-1` — انحراف إيصال PR #21 **واقعةً فقط** · دلتا حالة صفّ `OD-GOV-2` بنمط `OD-GOV-3` القائم — **والأقسام التاريخية لم يُعَد كتابتها**) · (3) `decisions/DEFERRED_IMPLEMENTATION.md` (‏+دلتا حالة راهنة لصفّ الإتاحة: التقنين وقع بـ`OD-R3-CODIFY-1` §9 · **المعالجة والاختبارات والأدلة ورفع التجميد وفكّ الحجب معلَّقة** بوجهة **أمر المعالجة التنفيذية المخصَّص** · **لا مسار `R` جديد** · سند «مسموح رغم التجميد» `:225` — **والصف التاريخي محفوظ**) · (4) `contracts/NAMING_AND_CONTRACTS_STANDARD.md` (‏`1.0 ⇒ 1.1` — §2 قواعد دورة الحياة الخمس القابلة لإعادة الاستعمال) · (5) `contracts/screens/SCREEN_CONTRACT_TEMPLATE.md` (‏`1.1 ⇒ 1.2` — «دورة الاعتماد» على المسار المحسوم `SA SIGNATURE OR APPROVED FP GATE` + دلالة `Approved` التوثيقية — **وبلا قيد الترحيل الخاص بالمشروع فيه**) · (6) `ui/UI_SCREEN_GOVERNANCE_STANDARD.md` (‏`1.0 ⇒ 1.1` — **52 ⇒ 51** · معرّفات الجرد الحالية قانونية · +§ج-5 إلزام التغطية) · (7) `ui/UI_SCREEN_BEHAVIOR_CARDS.md` (‏`1.2 ⇒ 1.3` — **قاعدة التغطية وحدها**: تغطية كاملة للـ51 بدل «25 والبقية عند الحاجة» — **ولم تُؤلَّف بطاقة**) · (8) `handoff/handoff.md` (هذا المدخل) · (9) `INDEX.md` (تحديث صفوف الملفات المعدَّلة + حواشي العقود الخمسة + **تصحيح سطر الإجمالي المتقادم**: **160 ملفاً + الفهرس** · **161 متتبَّعاً شاملةً الفهرس** · **160 صفَّ بيانات عدا الترويسة** · **`EC-3 = 161/161 = 100%`** — **بلا ملف جديد ⇒ بلا صفّ جديد**).
- **الأحكام المسجَّلة:** **دين `OD-R3-SCOPE-1` حُسم بلا ادّعاء معالجة** — حالة `R3-ACT-01` §(هـ) ⇒ `RESOLVED BY OD-R3-SCOPE-2` · الاستثناء المحدود ⇒ `SUPERSEDED FOR FUTURE EFFECT` · **الشروط القديمة (17)–(20) مستبدَلة** بالعشرة · **النص التاريخي المكرر في التتبع** ⇒ `HISTORICAL RECORD — SUPERSEDED FOR FUTURE EFFECT` **بلا تعديل للملف** · **`OD-GOV-2` محسوم** · **تعارض مسار الاعتماد محسوم** (`OD-R3-CONTRACTS-1` §(ب) ⇒ `RESOLVED`) · **انحراف إيصال PR #21 مسجَّل واقعةً فقط — وبلا تعديل `ADR-0033`**.
- not completed / **ما لم يُفعل عمداً**: **لا عقد رُقّي** — **والعقود الخمسة لا تُرقّى قبل ترحيلها إلى `UI-4`/`UI-5` وتحديث إحالاتها المتقادمة، والترحيلُ نفسُه يحجب التأليف الجماعي للعقود الـ46** · **لا تقنين `UI-4` ولا `UI-5`** (لكلٍّ أمرُ مالكٍ وPR مستقلان) · **لا بطاقة سلوك ولا عقد أُلّف** · **لا معالجة إتاحة ولا اختبار ولا أدلة** · **لا تعديل للجرد ولا للعقود الخمسة ولا للتتبع ولا لـ`decisions/adr/**`** · **`GOV-OBS-03` · `OD-GOV-1` · `OD-P3-1` · `OD-P3-2` (‏`CC-WF-1`) · `OD-P3-3` · `P3-CI-28` · `OD-NM-1` · `DD-WF-DETAIL-VIEW-PERM` مفتوحة بلا مساس** · **`FP-0001` = `BLOCKED`** · **التجميد = `ACTIVE` وشروطه (1)–(16) بلا تعديل ولا إعادة عدّ** · **`R4`/`R5` = `NOT_ACTIVATED`** · **`R3` لم تُغلق** · **لا وسم ولا إصدار ولا دمج**.
- decisions: **قرار مالك واحد جديد** = **`OD-R3-SCOPE-2`** (‏`RECORDED — EFFECTIVE UPON MERGE`). **لا ADR · لا إعادة ترقيم لأي `OD` قائم · لا طبقة سلطة جديدة.**
- risks: ‏(1) **قراءة حسم `OD-GOV-2` إذناً بالترقية** — مُعالَج بـ§(و)-5 و§(ز): لا ترقية قبل الترحيل. (2) **قراءة الإحلال المستقبلي محواً للتاريخ** — مُعالَج بنص §(ح): الملف لم يُعدَّل ونصوصه دليل وقتها. (3) **قراءة استبدال (17)–(20) تخفيفاً لشروط التجميد** — مُعالَج بـ§(د): (1)–(16) نافذة بلا مساس لغرضيها. (4) **قراءة سجل §(م) سياسةً أو معالجة** — مُعالَج بنصّه: واقعة فقط. (5) **قراءة تحديث قاعدة التغطية تأليفاً للبطاقات** — مُعالَج: لم تُؤلَّف بطاقة.
- tests: **لا بوابات برمجية في هذا المستودع** — والتحقق بأوامر `git`/`gh` وفحوص قرائية: تطابق خط الأساس حرفياً · نظافة الشجرة · **حصر التغيير في التسعة** · **byte-identity** للجرد A/B والعقود الخمسة والتتبع و`decisions/adr/**` مقابل `BASE_SHA` · بقاء 25 بطاقة و51 صفاً في كل جدول · `git diff --check` نظيف · `INDEX.md` **160 صفَّ بيانات** و**`EC-3 = 161/161`**.
- **المراجعة — تسجيل حالة:** جلسةُ التنفيذ أجرت **`Structured self-review`** فقط:
...
```
  ...
  **‏Execution session is not authorized to merge.**
  **ولا يجوز لوكيل التنفيذ الذي كتب هذه الدفعة مراجعتُها ولا دمجُها** (`agent-execution-model` §18.2-1 · `OD-GOV-MERGE-1` §(د)-3).
- next step: **العمل الحاكم التالي هو `UI-4`** (بأمر مالك مستقل وPR مستقل، ثم `UI-5` كذلك) · وقبل الدمج: **مراجعة مستقلة لهذا الـPR** على الرأس النهائي — جلسة/مراجعٌ لم يؤلّف الدفعة ولم يدفع commit إلى فرعها ⇒ `APPROVE` ⇒ **Merge Agent مستقل بأمر مالك صريح منفصل يثبّت `PR number` و`expected final head SHA`** (‏`ADR-0033` بند 10-(أ) وبند 11) — **وأمرُ التنفيذ الحالي ليس تفويض دمج**.
- do not touch: `constitution.md` · `AUTHORITY.md` · `CLAUDE.md` · `methodology/**` **عدا الصفّ المُواءم** · `phases/**` · `catalogs/**` · **`traceability/**` (ومنه `R3-ACCESSIBILITY-SPECIFICATION.md` رغم الإحلال المستقبلي)** · `references/**` · `knowledge/**` · `architecture/**` · `superseded/**` · **`decisions/adr/**` (ومنه `ADR-0033`)** · `contracts/enums/**` · `contracts/identity/**` · **العقود الخمسة `Candidate`** · **`ui/UI_SCREEN_INVENTORY.md`** · بقية `ui/**` خارج الملفين المأذونين · إعدادات المستودع وحماية الفروع وCI · أي وسم أو إصدار · أي سجل تفعيل `R4`/`R5` · `H-0016`..`H-0039`.
