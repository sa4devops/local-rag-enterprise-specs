...
- **سلّم المصطلحات (Δ 2026-07-11):** ‏**Platform → Products → Modules → Capabilities** — «المنصة» النواة المشتركة؛ «المنتج» تطبيق أعمال يُبنى فوقها من الميتاداتا؛ «الوحدة» حد منطقي داخل الـ Monolith ‏(ADR-0026)؛ «القدرة» وظيفة مسجلة في الـ Registry تُفعَّل بمرحلتها. قواعد التسمية تتبع الطبقة: أسماء الوحدات/القدرات/المعرّفات عامة الدلالة، و**يُمنع أي مفردات منتجٍ محدد داخل نواة المنصة** (أسماء إدارات/كيانات عميل بعينه تعيش في metadata المنتج لا في كود النواة أو مواصفاتها) — مصطلحات وقواعد تسمية فقط، بلا أي طبقات برمجية جديدة.
...
**جدول تفصيل D8 (إضافة توضيحية Δ 2026-07-11 — لا إعادة كتابة للسياسة):**
| نمط الاستخدام | الحكم | الشرط |
|---|---|---|
| دراسة تصميم/تجربة استخدام (صور/وثائق/تدفقات) | مسموح | تسجيل المرجع + سياسة المراجع البصرية (UI_REFERENCE_USAGE_POLICY) |
| دراسة APIs وعقود التكامل | مسموح | تسجيل المرجع في سجل المراجع |
| دراسة معمارية وأنماط وحواف | مسموح | ‏design-notes/summary — لا نسخ |
| إعادة استخدام أفكار/أنماط مُتعلَّمة (تنفيذ أصلي) | مسموح | معايير الحكم الأربعة أعلاه — النمط لا النص |
| إعادة استخدام كود مباشرة | مشروط | فقط حين تسمح السياسة والرخصة: صف license-review معتمد + امتثال كامل (notices/attribution) قبل الدمج |
