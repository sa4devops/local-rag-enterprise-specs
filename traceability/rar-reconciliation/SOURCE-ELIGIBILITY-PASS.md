...
> **HEAD المنفَّذ عليه المسح:** `52508598b53732215e5877ce8e2b1ea7993bf178` · **التاريخ:** 2026-07-27.
...
- الاستشهادات المصحَّحة: **21 ورودَ `INDEX.md:NNN`** + **9 اختصارات `(:NNN)`** تشير إلى أسطر الفهرس داخل جداول الأهلية — كلها حُوِّلت إلى الصيغة الهوياتية (التفصيل في `FINAL-RECONCILIATION-TABLE.md` §5).
...
1. **`phases/BACKLOG_DEFERRED_SCOPE.md` غير مرتَّب في السلّم.** حالته `Accepted (v0.3)` وهو سجلُّ نطاقٍ مؤجَّل بقرار، لكن `AUTHORITY.md` يرتّب من `phases/` **الخارطة والخطة الأم (المرتبة 6)** و**التصاميم (المرتبة 7)** فقط — ولا يذكره. **12 صفاً** يستند إليه (‏RAR-002 `[08]` · RAR-003 `[30]` · تسعة في RAR-005). ونصّه يقيّد نفسه: «مرجع تاريخي معتمد … **لا تُستخدم للتنفيذ المباشر**» و«ما استجد … يعلوها عند أي تعارض (م20)». **لم يُستعمل وحده لإثبات أي `مقنَّن`** (تحقّق آلي)، واستُعمل حصراً لإثبات **تأجيلٍ بقرار** — وهو ما ينص عليه نصاً. **الملاحظة مرفوعة**: إدراجُه في السلّم أو تفويضُه من مرتبة أعلى **قرارُ مالك**، ولم يُتخذ هنا.
