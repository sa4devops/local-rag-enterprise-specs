...
| OpenWebUI | v0.6.5 للدراسة فقط | github.com/open-webui | BSD-3-Clause ‏(v0.6.5) | **reference only — ليس dependency** | – | – | نعم | – | – | متوسط | **Conditional (مرجع فقط)** | لا يُنسخ كوده؛ خاضع لسياسة Reference-aware (ADR-0021)؛ قرار الترخيص خارج نطاق هذه الدفعة ويعود لمالك المشروع عند اعتماد واجهة التشغيل. |
| OpenWebUI (container image — interim chat client) | **v0.6.5 مثبَّت (لا latest)** — digest يُسجَّل إلزامياً عند أول سحب/اعتماد تشغيلي | ghcr.io/open-webui/open-webui | BSD-3-Clause ‏(v0.6.5) | container image (واجهة محادثة مؤقتة — ADR-0030) | لا (تشغيل كما هي — لا fork ولا نسخ كود) | لا (استخدام داخلي) | **نعم عند أي white-label — ممنوع** | نعم | نعم (إبقاء الهوية/الإشعارات) | متوسط | **Needs Legal Review** | قرار الترخيص خارج نطاق هذه الدفعة ويعود لمالك المشروع عند اعتماد واجهة التشغيل. |
