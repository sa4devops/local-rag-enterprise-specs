...
> **Version:** 1.1 (صفوف الستاك المعتمد + سياسة المراجع) · **Status:** Current / Accepted · **Date:** 2026-07-04 · **Authority:** ينفّذ constitution م7 وMethodology §15.
...
|---|---|---|---|---|---|---|---|---|---|---|---|---|
| PostgreSQL | (حدِّد) | postgresql.org | PostgreSQL | container image | لا | نعم | لا | نعم | لا | منخفض | Needs Review | قواعد ×4 بمستخدم least-privilege لكل قاعدة |
| Qdrant | (حدِّد) | qdrant.tech | Apache-2.0 | container image | لا | نعم | لا | نعم | نعم (NOTICE) | منخفض | Needs Review | حاضر في Local Demo (ADR-0004) |
| OpenSearch | (حدِّد) | opensearch.org | Apache-2.0 | container image | لا | نعم | لا | نعم | نعم | منخفض | Needs Review | حاضر في Local بإعداد heap خفيف |
| Redis | (حدِّد) | redis.io | RSAL/SSPL (متغيّر) | container image | لا | مقيّد | لا | نعم | — | **مرتفع** | **Needs Legal Review** | فئة 3؛ البديل Valkey |
| Valkey | (حدِّد) | valkey.io | BSD-3 | container image | لا | نعم | لا | نعم | لا | منخفض | Needs Review | **optional-ready (D4/ADR-0018)** — يُفعَّل عند الحاجة بعد المراجعة؛ الافتراضي memory/PostgreSQL |
| MinIO | (حدِّد) | min.io | AGPL-3.0 | container image | لا | AGPL شروط | لا | نعم | نعم | **مرتفع عند التوزيع** | **Needs Legal Review** | داخلياً غالباً مقبول؛ الموقف النهائي قبل أي release (فئة 4) |
| SeaweedFS (بديل) | (حدِّد) | seaweedfs | Apache-2.0 | container image | لا | نعم | لا | نعم | نعم | منخفض | Needs Review | بديل S3-compatible إن رُفض MinIO |
...
| SQLite | (حدِّد) | sqlite.org | Public Domain | dependency (tests) | لا | نعم | لا | نعم | لا | منخفض | Needs Review | اختبارات/محلي فقط |
| Alembic | (حدِّد) | sqlalchemy.org | MIT | dependency | لا | نعم | لا | نعم | لا | منخفض | Needs Review | أداة الترحيل داخل غلاف الحوكمة |
| FastAPI (+Starlette/Pydantic) | (حدِّد) | fastapi.tiangolo.com | MIT | dependency | لا | نعم | لا | نعم | لا | منخفض | Needs Review | Backend framework |
| React / Vite / TypeScript | (حدِّد) | react.dev / vitejs.dev / typescriptlang.org | MIT / Apache-2 | dependency + build tool | لا | نعم | لا | نعم | لا | منخفض | Needs Review | ستاك الواجهة المعتمد (D1/ADR-0017)؛ **Next.js غير معتمد** |
| Tailwind CSS | (حدِّد) | tailwindcss.com | MIT | dependency | لا | نعم | لا | نعم | لا | منخفض | Needs Review | RTL عبر logical properties |
| shadcn/ui | (حدِّد) | ui.shadcn.com | MIT | **copy-in** | نعم (ننسخ ونعدّل) | نعم | لا | نعم | لا | منخفض | Needs Review | مكوّنات تُنسخ للمستودع |
| uv | (حدِّد) | astral.sh/uv | MIT/Apache-2 | dev tool | لا | نعم | لا | نعم | لا | منخفض | Needs Review | مدير حزم Python المعتمد (D3) + offline cache |
| pnpm | (حدِّد) | pnpm.io | MIT | dev tool | لا | نعم | لا | نعم | لا | منخفض | Needs Review | مدير حزم الواجهة المعتمد (D3) + offline store |
...
| Docker Engine / Compose | (حدِّد) | docker.com | Apache-2.0 | dev/ops tool | لا | نعم | لا | نعم | — | منخفض | Needs Review | أساس النشر |
...
**سياسة المراجع مفتوحة المصدر (Reference-aware — D8/ADR-0021):** قراءة/دراسة كود OSS للتعلم والفهم والمقارنة مسموحة دون قيد. **أي إعادة استخدام مباشرة لكود خارجي** — ملف كامل، مقتطف يتجاوز ~10 أسطر، أو بنية منسوخة بأسماء متغيّرات مُبدَّلة (بما فيه الاستنساخ الحرفي «من ذاكرة النموذج») — تتطلب صفاً في هذا السجل وامتثال ترخيص صريحاً، مع **حفظ notices/attribution وعدم إزالتها**. كود AGPL/SSPL/copyleft/commercial لا يدخل إلا بـ Legal Review. أوزان النماذج تُعامل كـ dependencies بهذا السجل. استخدام مرجع لأي مهمة يُذكر في handoff.
